...
int start(){
   ...
   for(i=limit;i>=0;i--){
      ...
      double gap = 0.02;
      // if signal UP (CALL)
      if (signal(up) && signal(up2)&& signal(up3)&& signal(up4)&& signal(up5)&& signal(up6)&& signal(up7)&& signal(up8)&& signal(up9)&& signal(up10)) {
      ...
      // if signal DOWN (PUT)
      if (signal(dn) && signal(dn2)&& signal(dn3)&& signal(dn4)&& signal(dn5)&& signal(dn6)&& signal(dn7)&& signal(dn8)&& signal(dn9)&& signal(dn10)) {
